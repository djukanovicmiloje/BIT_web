...
function generateProductElement(product) {
  ...
  addToCart.addEventListener("click", () => { korpa.addProduct(product); updateKorpa() })
...
for (var i = 0; i < addBtns.length; i++) {
  var product = addBtns[i].parentNode.product;
  console.log(product);
  addBtns[i].addEventListener("click", () => {
    korpa.addProduct(addBtns[parseInt(i)].parentNode.product); console.log(korpa);
  });
}
console.log(i);
console.log(product);
